...
class scanner:
    def __init__(self, group, robot, dist_trunk, scan_radius, scan_angle, num_levels, level_offset, velocity=0.1):
        ...
        self.ee_joint = 6 # 6 on real robot
        self.camera = Camera()
    ...
    def go_home(self, mode):
        if mode=='bot':
            joint_target = [0.0, 0.0014396050246432424, -1.374225918446676, 1.3128395080566406, -1.5089119116412562, 1.5708682537078857, 3.141312599182129]
        elif mode=='top':
            joint_target = [0.0, -0.0633624235736292, -0.5650656859027308, -2.0108359495746058, -2.1355107466327112, -1.569862190877096, 3.1410374641418457]
    ...
    def set_home(self):
        ...
        joint_target[self.ee_joint] = np.pi
        self.group.set_joint_value_target(joint_target) 
        plan = self.group.plan() 
    ...
    def execute_plan_by_joints(self, plan, view_points=16):
        num_waypoints = len(plan.joint_trajectory.points)
        step = int(num_waypoints/view_points)
        joint_waypoints = []
    ...
    def generate_cylindrical_waypoints(self, num_levels):
